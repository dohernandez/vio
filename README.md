...
In case you wanna test the service, you do the following:

**Note:** Remember to have the `.env` file properly configured.

- Run the services by doing `make start-dc`. This will automatically generate the binaries (server and cli) under the folder `/bin`.

- After that run the command `make build` to generate the `vio` properly. Sometimes using the binary generated by the `make start-dc` can cause some issues. Make sure command works first by running `./bin/vio --help`.

- Parse the sample data with the cli binary by doing:
```
...
```
The parser can be configured to parse data in parallel by using the `-p` flag. For more information about the cli, you can do `./bin/vio parse filesystem --help`.
 With the script `./resources/app/scripts/env-run.sh` in from we make sure that the env variables defined in the `.env` file are available for the cli.
